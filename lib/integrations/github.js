...
module.exports = class GitHubIntegration {
	...
	async loadContractFromPR (github, event, prData) {
		...
		try {
			const contractFile = await githubRequest(github.repos.getContent, {
				owner: orgName,
				repo: repoName,
				path: 'balena.yml',
				ref: prData.head.sha
			...
			if (!contractFile.data || contractFile.data.type !== 'file' || !contractFile.data.size) {
				this.context.log.warn(`balena.yml in ${commitRef} is not a valid file`)
				return null
			}

			const contract = YAML.parse(Buffer.from(contractFile.data.content, 'base64').toString('utf-8'))
			if (!contract.type) {
				this.context.log.warn(`balena.yml in ${commitRef} is not a valid contract`)
				return null
			}
			return contract
		} catch (err) {
	...
	async translate (event) {
		...
		switch (type) {
			case 'check_run': {
				...
				if (checkRun.external_id.match(/^jellyfish_[A-F\d]{8}-[A-F\d]{4}-4[A-F\d]{3}-[89AB][A-F\d]{3}-[A-F\d]{12}$/i)) {
				...
				const currentContract = await this.context.getElementBySlug(`${card.slug}@1.0.0`)
