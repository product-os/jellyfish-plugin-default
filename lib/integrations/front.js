...
const getMessage = async (instance, front, intercom, sequence, payload, threadId, emittedDate, remoteMessages) => {
	...
	const currentMessage =
		await getElementByFuzzyMirrorId(instance.context, type, mirrorId)
 ...
 */
const getThreadPatchFromEvent = (event) => {
	const patch = []
...
const getThread = async (context, front, event, inbox, threadType) => {
	...
	const threadCard =
		await getElementByFuzzyMirrorId(context, threadType, mirrorId)
	if (threadCard) {
		return threadCard
	}
...
module.exports = class FrontIntegration {
	...
	async translate (event) {
		...
		const patch = getThreadPatchFromEvent(event)
		const updatedThreadCard = jsonpatch.applyPatch(threadCard, patch)
