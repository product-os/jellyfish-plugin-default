...
const getCardSlug = (type, name) => {
	return `${type.split('@')[0]}-${utils.slugify(name || uuid())}`
}
...
module.exports.slug = 'balena-api'

module.exports.mergeCardWithPayload = mergeCardWithPayload

module.exports.isEventValid = async (token, rawEvent, headers, context) => {
	if (!token) {
		return false
	}
...
module.exports.whoami = async (context, credentials, options, retries = 10) => {
	const {
		...
		(resolve, reject) => {
			axios.get(`${module.exports.OAUTH_BASE_URL}/user/v1/whoami`,
				{
					headers: {
